...
- **설정 탭**: API 키 입력, 채널 핸들로 등록/삭제 (`channels.json`에 저장되어 다음 실행 때도 유지됨), 접속 코드가 설정되어 있으면 접속 시도 기록도 확인 가능
...
- **접속 코드 게이트**: Secrets에 `ACCESS_CODE`를 등록해두면, 이 코드를 아는 사람만 화면(사이드바 포함) 전체를 볼 수 있습니다. 브라우저를 새로고침하거나 새 탭에서 열면 코드를 다시 입력해야 합니다 (세션에만 저장, 영구 로그인 아님). 5번 연속 틀리면 60초간 잠기고, 성공/실패 시도는 설정 탭에서 확인할 수 있습니다.
...
3. 배포 전/후 **Settings → Secrets**에 아래 내용 추가 (`.streamlit/secrets.toml.example` 참고):
   ...
   ```
4. 배포되면 링크(`https://xxxx.streamlit.app`)를 공유하고 싶은 사람에게 URL + `ACCESS_CODE`를 전달
5. 접속하면 코드 입력 화면이 먼저 뜨고, 맞는 코드를 입력해야만 안쪽 화면(사이드바 포함)이 보입니다. API 키 값 자체는 화면 어디에도 노출되지 않습니다.

> **접속 코드 게이트의 특징**: 브라우저를 새로고침하거나 새 탭/새 기기에서 열면 코드를 다시 입력해야 합니다 (로그인 쿠키 같은 영구 저장이 아니라 그 세션에서만 유지됨). 링크를 아는 사람 누구나 코드만 맞으면 들어올 수 있는 방식이라, "이 사람은 되고 저 사람은 안 되고"처럼 개인별로 막고 싶다면 Streamlit Cloud 자체의 **Settings → Sharing → Private + 이메일 초대** 기능을 대신(또는 같이) 쓰세요 — 그러면 초대된 구글 계정으로 로그인해야만 접속 가능해집니다.
>
...
- **접속 코드는 개인별이 아닌 공용 비밀번호**: 특정 한 사람만 차단하려면 코드를 바꿔서 모두에게 다시 알려줘야 합니다. 개인별 접근 제어가 필요하면 Streamlit Cloud의 Private + 이메일 초대 기능을 쓰세요.
